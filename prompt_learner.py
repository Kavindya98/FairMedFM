...
class ClipPromptClassifier(nn.Module):
    ...
    def __init__(self, model: nn.Module, class_names: List[str],  device=None, template="a photo of a {}"):
        super(ClipPromptClassifier, self).__init__()
        
        self.class_names = [" ".join(c.split("_")) for c in class_names]
        self.model = model
        self.text_inputs = torch.cat([clip.tokenize(template.format(c.strip())) for c in self.class_names]).to(device) 
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        text_features = self.model.text_encoder(prompts, self.prompt_learner.tokenized_prompts)                            
